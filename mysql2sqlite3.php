...
function mysql_query ($query, $link_identifier = null) {
    ...
    if (preg_match('/^SELECT.*/i', $query) == 0) {
        if ($link_identifier->exec($query) === true) {
            return $link_identifier->changes();
        }
        else {
            return false;
        }
...
function mysql_error ($link_identifier = null) {
    ...
    return $link_identifier->lastErrorMsg();
}
...
function mysql_errno ($link_identifier = null) {
    ...
    return $link_identifier->lastErrorCode();
}
...
function mysql_insert_id ($link_identifier = null) {
    ...
    return $link_identifier->lastInsertRowID();
}
...
function mysql_result ($result, $row, $field = null) {
	...
	if ($field == null)
		$field = 0;
	$i = 0;
	while ($x = $result->fetchArray(SQLITE3_NUM)) {
		//print_r($x);
		if ($i == $row) {
...
function mysql_fetch_array ($result, $result_type = null) {
    $sqlite3_result_mode = SQLITE3_BOTH;
    switch ($result_type) {
        case MYSQL_BOTH:
            $sqlite3_result_mode = SQLITE3_BOTH;
            break;
        case MYSQL_ASSOC:
            $sqlite3_result_mode = SQLITE3_ASSOC;
            break;
        case MYSQL_NUM:
            $sqlite3_result_mode = SQLITE3_NUM;
            break;
    }
    return $result->fetchArray($sqlite3_result_mode);
}
...
function mysql_field_name ($result, $field_offset) {
    return $result->columnName($field_offset);
}
...
function mysql_escape_string ($unescaped_string) {
    return SQLite3::escapeString($unescaped_string);
}
